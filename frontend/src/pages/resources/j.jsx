...
export default function OSCEDetailPage() {
  ...
  // Ref for the main container – used to apply no‑screenshot class
  ...
  useEffect(() => {
    ...
    // 2. Block common screenshot/save/print shortcuts
    const handleKeyDown = (e) => {
      // Ctrl+S, Ctrl+P
      if ((e.ctrlKey || e.metaKey) && (e.key === 's' || e.key === 'S' || e.key === 'p' || e.key === 'P')) {
        e.preventDefault();
        // Optional: show a brief notification
        // toast.warn("Saving is disabled on this page");
        return;
      }
      // Ctrl+Shift+S, Ctrl+Shift+P
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.key === 'S' || e.key === 'P')) {
        e.preventDefault();
        return;
      }
      // F12 (Developer Tools)
      ...
      // PrintScreen key (may not fire in all browsers, but we try)
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className="flex justify-center bg-gray-100 dark:bg-gray-900 min-h-[calc(100vh-64px)] p-4 text-gray-900 dark:text-gray-100"
